...
function ProfileView() {
    ...
    useEffect(() => {
        ...
        if(window && typeof window != "undefined") {
            setWindow(window);
            if(!window.localStorage.getItem("mail") || !window.localStorage.getItem("password") || !window.localStorage.getItem("settings")) {
               window.location.href = "/register"
               
            }
            let localSettings = window.localStorage.getItem("settings")
            let settings:any = JSON.parse(localSettings as any)
            setName(settings?.fullName || "")
            setFeaturedTitle(settings?.featuredTitle || "")
            setTitle(settings?.title || "")
            
        }

        /*
        axios.instance({ baseUrl: DEFAULTS.SERVER_ENDPOINT})
        axios.post("/register", {
        body : {
            email: 
        }
